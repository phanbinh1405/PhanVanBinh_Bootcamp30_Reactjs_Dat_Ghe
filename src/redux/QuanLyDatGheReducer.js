	...
	danhSachGhe: [
		{
			...
			danhSachGhe: [
				...
				{ soGhe: "A4", gia: 75000, daDat: true },
				{ soGhe: "A5", gia: 75000, daDat: false },
				{ soGhe: "A6", gia: 75000, daDat: true },
...
export const QuanLyDatGheReducer = (state = defaultSate, action) => {
	switch (action.type) {
		case "CHON_GHE": {
			const currentSeat = action.payload;
			let chooseSeat = state.gheDangChon.findIndex(
				(ghe) => ghe.soGhe === currentSeat.soGhe
			);
			let gheDangChonClone = [...state.gheDangChon];

			if (chooseSeat !== -1) {
				gheDangChonClone.splice(chooseSeat, 1);
			} else {
				gheDangChonClone.push(currentSeat);
			}

			return { ...state, gheDangChon: gheDangChonClone };
		}
